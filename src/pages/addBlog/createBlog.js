...
const {fetchData } = useTheme();
  ...
  <div className="lg:w-3/4 flex justify-center items-center">
      ...
      <div className="text-center lg:text-left ml-10">
        <h2 className="text-3xl font-bold">Say Anything!</h2>
        <p className="py-6">Explore the depths of human emotion and experience with our latest blog post. Delve into captivating narratives and thought-provoking insights, offering a glimpse into the diverse facets of life. Engage with stories that resonate and inspire, as we navigate through the complexities of existence together.</p>
      </div>
    </div>
    <div className="lg:w-3/4">
      ...
      <div className="card shadow-2xl bg-base-100">
        <form className="card-body" onSubmit={handleSubmit}>
          <h1 className="text-5xl font-bold mb-6">Add a Blog!</h1>
          <div className="form-control">
            <label className="label">
              <span className="label-text">Title</span>
            </label>
            <input type="text"  value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enter title" className="input input-bordered" required />
          </div>
          <div className="form-control">
            <label className="label">
              <span className="label-text">Description</span>
            </label>
            <textarea placeholder="Bio" value={description} onChange={(e) => setDescription(e.target.value)} className="textarea textarea-bordered textarea-lg w-full max-w-xs" ></textarea>  
          </div>
